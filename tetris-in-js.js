...
var hump = [[0,0],[1,0],[-1,0],[0,1]];
var zShape = [[-1,1],[0,1],[0,0],[1,0]];
var zShapeReversed = [[-1,0],[0,0],[0,1],[1,1]];
/************************
    YOUR CODE ENDS
*************************/
...
function moveLeft(piece) {
    ...
    *************************/
    for(var i=0; i<4; i++){
        if (piece.colPos + piece.layout[i][1] == 0 || occupationMatrix[piece.rowPos + piece.layout[i][0]][piece.colPos + piece.layout[i][1] - 1] != null){
            return;
        }
    }
...
function moveRight(piece) {
    ...
    *************************/
    for(var i=0; i<4; i++){
        if (piece.colPos + piece.layout[i][1] == COL_NUM-1 || occupationMatrix[piece.rowPos + piece.layout[i][0]][piece.colPos + piece.layout[i][1] + 1] != null){
            return;
        }
    }
    setPosition(piece,piece.rowPos,piece.colPos + 1);
...
function isMoveDownValid(piece) {
    ...
    */
    ...
    /************************
        YOUR CODE STARTS
    *************************/
    var row = piece.rowPos;
    var col = piece.colPos;
    for (var i = 0; i < 4; i++){
    if (row+piece.layout[i][0] >= ROW_NUM-1)
        return false;
    }
    for (var i = 0; i < 4; i++){
        if (occupationMatrix [row+piece.layout[i][0]+1][col+piece.layout[i][1]] != null)
            return false;
    }
    return true;
